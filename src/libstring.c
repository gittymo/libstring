...
char *createString(const char *CHAR_ARRAY)
{
    ...
    if (CHAR_ARRAY)
        strcpy(string_char_ptr, CHAR_ARRAY);
...
bool isString(const char *STRING, _StringHeader **_string_header_ptr)
{
    ...
    if (_string_header_ptr)
        *_string_header_ptr = header;
...
{
    if (OUTPUT_STRING_FORMAT == STRING_FORMAT_UNDEFINED)
        OUTPUT_STRING_FORMAT = STRING_FORMAT_AS_IS;
    ...
    {
        while (start < end && isblank(STRING[start]))
            start++;
    }
    if (OUTPUT_STRING_FORMAT & STRING_FORMAT_TRIM_RIGHT)
    {
        while (end > start && isblank(STRING[end]))
            end--;
    }
    ...
    {
        size_t si = i + start;
        if ((OUTPUT_STRING_FORMAT & STRING_FORMAT_AS_IS) == STRING_FORMAT_AS_IS)
            string_copy[i] = STRING[si];
        else if (OUTPUT_STRING_FORMAT & STRING_FORMAT_LOWERCASE)
            string_copy[i] = STRING[si] + (32 * (STRING[si] >= 'A' && STRING[si] <= 'Z'));
        else if (OUTPUT_STRING_FORMAT & STRING_FORMAT_UPPERCASE)
            string_copy[i] = STRING[si] - (32 * (STRING[si] >= 'a' && STRING[si] <= 'z'));
    }
    return string_copy;
}
...
bool compareStrings(const char *STRING1, const char *STRING2, size_t STRING_COMPARISON_FORMAT)
{
    ...
    if (!isString(STRING1, &string1Header) || !isString(STRING2, &string2Header))
        return false;
    if (string1Header->length != string2Header->length)
        return false;
    bool match = true;
    for (size_t i = string1Header->length; match && i--;)
    {
        if (STRING_COMPARISON_FORMAT != STRING_FORMAT_CASE_INSENSITIVE)
...
{
    if (!string_tokens_array || string_tokens_array_length < 1)
        return;
...
{
    if (OUTPUT_TOKEN_STRING_FORMAT == STRING_FORMAT_UNDEFINED)
        OUTPUT_TOKEN_STRING_FORMAT = STRING_FORMAT_AS_IS;

    _StringHeader *header;
    if (!isString(STRING, &header) || !DELIMETERS_CHAR_ARRAY || !tokens_array_size_ptr)
        return NULL;
    if (header->length == 0)
        return NULL;
...
size_t getStringHash(const char *STRING)
{
    ...
    int *UINT_DATA_PTR = (unsigned int *)STRING;
    int i = 0;
